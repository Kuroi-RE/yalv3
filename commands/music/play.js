...
const forHumans = require("../../Database/data");

module.exports = {
  name: "play",
  aliases: ["p"],
  usage: ["[nama musik]"],
  description: "Mainkan sebuah lagu",
  async execute(message, args) {
    ...
    if (!channel)
      return error("Kamu harus berada dalam channel untuk memainkan lagu!");

    if (!channel.permissionsFor(message.client.user).has("CONNECT"))
      return error("Saya tidak memiliki akses untuk masuk kedalam channel.");
    ...
    if (!query)
      return error("Kamu tidak memberikan nama lagu untuk dimainkan!");

    if (query.includes("www.youtube.com")) {
      try {
        const ytdata = await await yt.getBasicInfo(query);
        if (!ytdata)
          return error("Tidak dapat menemukan lagu dari Url tersebut");
        song = {
      ...
      try {
        ...
        if (fetched.length === 0 || !fetched)
          return error("Saya tidak menemukan lagu yang anda cari!");
    ...
    if (list) {
      ...
      return send(
        new MessageEmbed()
          .setAuthor(
            "Lagu telah dimasukan ke dalam Playlist",
    ...
    async function play(track) {
      try {
        ...
        if (!track) {
          data.channel.send("Lagu telah habis, Saya Keluar dari saluran.");
        ...
        data.channel.send(
          new MessageEmbed()
            ...
            .setFooter("Another Life Music Bots")
        );
      } catch (e) {
        console.error(e);
      }
